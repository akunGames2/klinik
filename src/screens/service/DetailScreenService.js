...
import { delete_service_data, get_detail_service_data } from '../../store/actions/serviceAction'
...
class DetailScreenMedicine extends BaseScreen {
  ...
  async saveInput(data) {
    if(data == 'EDIT') {
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_SERVICE_UPDATE, {
        itemId: this.id
      })
    } else if(data == 'DELETE') {
      this.delete()
  ...
  async delete() {
    const result = await this.props.dispatch(delete_service_data(this.id))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_SERVICE)
    }else {
      this.showError(result.getMessage())
    }
  }
  ...
  async get_detail_data() {
    let response = await this.props.dispatch(
      get_detail_service_data(this.id)
    )

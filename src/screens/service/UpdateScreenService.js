...
import { get_detail_service_data, update_service } from '../../store/actions/serviceAction'

class UpdateScreenService extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},            
  ...
  async saveInput() {
    ...
    const result = await this.props.dispatch(update_service(data))
    if (result.isSuccess()) {
  ...
  async get_detail_data() {
    ...
    if(response.isSuccess()) {      
      ...
      this.setState({ 
        jenis_layanan: data.nama_layanan, tarif: data.tarif, keterangan: data.keterangan
      })
    } else {
...
UpdateScreenService.navigationOptions = ({ navigation }) => ({
  title: 'UPDATE DATA',

...
import { delete_medicine_data, get_detail_medicine_data } from '../../store/actions/medicineAction'
...
class DetailScreenMedicine extends BaseScreen {
  ...
  async saveInput(data) {
    if(data == 'EDIT') {
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_MEDICINE_UPDATE, {
        itemId: this.id
      })
    } else if(data == 'DELETE') {
      this.delete()
  ...
  async delete() {
    const result = await this.props.dispatch(delete_medicine_data(this.id))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_MEDICINE)
    }else {
      this.showError(result.getMessage())
    }
  }

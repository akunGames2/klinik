...
class UpdateCardMotherScreen extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},            
  ...
  async saveInput() {
    ...
    const result = await this.props.dispatch(update_cardmother(data))
    if (result.isSuccess()) {
  ...
  async get_detail_data() {
    ...
    if(response.isSuccess()) {      
      ...
      this.setState({ 
        id_kartu_ibu:data.id_kartu_ibu,id_profil_suami:data.id_profil_suami, id_riwayat_perkawinan:data.id_riwayat_perkawinan, id_riwayat_persalinan: data.id_riwayat_persalinan, id_riwayat_kehamilan:data.id_riwayat_kehamilan, id_pemeriksaan:data.id_pemeriksaan, id_rencana_persalinan: data.id_rencana_persalinan,
        datePickerExpiry: {value:data.tgl_daftar },datePickerPasang: {value:data.tgl_pasang},datePickerKartuIbu: {value:data.tgl_kartu_ibu}, bps_rb: data.bps_rb, desa: data.desa, kota: data.kota, kode_index:data.kode_index, id_bidan: data.id_bidan, bidan: data.nama_bidan, id_pasien: data.id_pasien, pasien: data.nama_pasien, id_layanan: data.id_layanan, nama_layanan: data.nama_layanan, nama_suami: data.nama_suami, usia: data.usia, agama: data.agama, pekerjaan:data.pekerjaan, pendidikan: data.pendidikan, kawin_ke: data.kawin_ke, lama_kawin:data.lama_kawin, sebab_pisah: data.sebab_pisah, sebab_mati: data.sebab_mati, hamil_ke: data.hamil_ke, komplikasi_hamil:data.komplikasi_hamil, jenis_persalinan: data.jenis_persalinan, tempat: data.tempat, komplikasi:data.komplikasi, penolong: data.penolong, jk_bayi:data.jk_bayi, berat_bayi: data.berat_bayi, kondisi_bayi_lahir:data.kondisi_bayi_lahir, kondisi_bayi_skrg: data.kondisi_bayi_skrg, kb:data.kb, asi:data.asi, haid:data.haid,hpht: data.hpht, hpl:data.hpl, berat:data.berat, mual_muntah: data.mual_muntah, pusing: data.pusing, nyeri_perut:data.nyeri_perut, gerak_janin: data.gerak_janin, oedema: data.oedema, nafsu_makan:data.nafsu_makan, pendarahan: data.pendarahan, penyakit_bumil:data.penyakit_bumil,penyakit_keluarga:data.penyakit_bumil, kebiasaan_bumil:data.kebiasaan_bumil, status_tt:data.status_tt,hiv_aids:data.hiv_aids,keluhan:data.keluhan, TB:data.TB, LILA:data.LILA, bentuk_tubuh: data.bentuk_tubuh, kesadaran: data.kesadaran, muka: data.muka, kulit: data.kulit, mata:data.mata, mulut:data.mulut, gigi:data.gigi, pembesaran_kel: data.pembesaran_kel, dada:data.dada, nafas:data.nafas, jantung:data.jantung, payudara:data.payudara, tangan_tungkai:data.tangan_tungkai, reflek:data.reflek,penolong:data.penolong, tempat_bersalin:data.tempat_bersalin, pendamping:data.pendamping, pendonor:data.pendonor, sticker_p4k:data.sticker_p4k, diagnosa:data.diagnosa
      })
    } else {
...
UpdateCardMotherScreen.navigationOptions = ({ navigation }) => ({
  title: 'UPDATE DATA',

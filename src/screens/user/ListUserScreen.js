...
import { get_list_user_data } from '../../store/actions/UserAction'

class ListUserScreen extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},
        ...
        <View style={[Styles.outerContainer,{backgroundColor: 'white'}]}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // let id = l.id_alkes
                this.props.navigation.navigate(NavKey.USER_DETAIL, {
                  itemId: l.id_user
                })
              }}
            >
          ...
          {this.state.permission && this.state.permission.create && <FloatingActionButton 
            navigation={() => this.props.navigation.navigate(NavKey.USER_ADD)}
            name="add"
            iconType="Ionicons"
          />}
        </View>
...
ListUserScreen.navigationOptions = ({ navigation }) => ({
  title: 'DATA USER',

...
import { delete_tool_data, get_detail_tool_data } from '../../store/actions/toolsAction'
...
class DetailScreen extends BaseScreen {
  ...
  async saveInput(data) {
    if(data == 'EDIT') {
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_TOOL_HEALTH_UPDATE, {
        itemId: this.id
      })
    } else if(data == 'DELETE') {
      this.delete()
  ...
  async delete() {
    const result = await this.props.dispatch(delete_tool_data(this.id))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_TOOL_HEALTH)
    }else {
      this.showError(result.getMessage())
    }
  }

...
class ListReportScreen extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},
        ...
        <View style={[Styles.outerContainer,{backgroundColor: 'white'}]}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // let id = l.id_alkes
                // this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_TOOL_HEALTH_DETAIL, {
                //   itemId: l.id_kategori_laporan
                // })
                this.get_report(l.url)
              }}
            >
              <View
                style={{
              ...
              >
                ...
                >
                  <Text style={{color: 'white',fontSize: 20}}>{l.nama_kategori}</Text>
                </View>
                <View
                  style={{
                    flex:1, 
  ...
  async get_report(data) {
    Linking.openURL(data).catch(err => (this.showError("Couldn't load page",err)));
  }
  ...
  async init() {
    let data = await this.get_permission('laporan')
...
ListReportScreen.navigationOptions = ({ navigation }) => ({
  title: 'LAPORAN',

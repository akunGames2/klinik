...
import { get_data_cardbaby } from '../../store/actions/cardbabyAction'
import { get_data_cardkb } from '../../store/actions/cardkbAction'

class ListCardKbScreen extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},
        ...
        <View style={[Styles.outerContainer,{backgroundColor: 'white'}]}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // let id = l.id_alkes
                this.props.navigation.navigate(NavKey.CARD_KB_DETAIL, {
                  itemId: l.id_kartu_kb
                })
              }}
            >
              <View
              ...
              >
                ...
                >
                  <Text>{l.nama}</Text>
                  <Text>{l.tgl_dilayani}</Text>
                  <Text>{l.metode_kb}</Text>
                </View>
                <View
                  style={{
                    flex:1, 
          ...
          {this.state.permission && this.state.permission.create && <FloatingActionButton 
            navigation={() => this.props.navigation.navigate(NavKey.CARD_KB_ADD)}
            name="add"
            iconType="Ionicons"
          />}
        </View>
  ...
  async onRefresh() {
    const response = await this.props.dispatch(
      get_data_cardkb(this.props.lp__user_id)
    )
...
ListCardKbScreen.navigationOptions = ({ navigation }) => ({
  title: 'DATA KARTU KB',

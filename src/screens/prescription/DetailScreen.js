...
import { delete_presciption_data, get_detail_presciption_data } from '../../store/actions/prescriptionAction'
...
class DetailScreen extends BaseScreen {
  ...
  async saveInput(data) {
    if(data == 'EDIT') {
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PRESCRIPTION_UPDATE, {
        itemId: this.id
      })
    } else if(data == 'DELETE') {
      this.delete()
  ...
  async delete() {
    const result = await this.props.dispatch(delete_presciption_data(this.id, this.state.id_detil_resep))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PRESCRIPTION)
    }else {
      this.showError(result.getMessage())
    }
  }

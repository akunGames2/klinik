...
class ListCardBabyScreen extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},
        ...
        <View style={[Styles.outerContainer,{backgroundColor: 'white'}]}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                // let id = l.id_alkes
                this.props.navigation.navigate(NavKey.CARD_BABY_DETAIL, {
                  itemId: l.id_kartu_bayi
                })
              }}
            >
              <View
              ...
              >
                ...
                >
                  <Text>{l.nama_bayi}</Text>
                  <Text>{l.tgl_lahir}</Text>
                  <Text>{l.nama_ibu}</Text>
                </View>
                <View
                  style={{
                    flex:1, 
          ...
          {this.state.permission && this.state.permission.create && <FloatingActionButton 
            navigation={() => this.props.navigation.navigate(NavKey.CARD_BABY_ADD)}
            name="add"
            iconType="Ionicons"
          />}
        </View>
  ...
  async onRefresh() {
    const response = await this.props.dispatch(
      get_data_cardbaby(this.props.lp__user_id)
    )
...
ListCardBabyScreen.navigationOptions = ({ navigation }) => ({
  title: 'DATA KARTU BAYI',

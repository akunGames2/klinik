...
import { get_detail_medicine_data, get_detail_purchase_medicine_data, update_medicine, update_purchase_medicine } from '../../store/actions/medicineAction'
...
class UpdateScreenMedicine extends BaseScreen {
  ...
  async saveInput() {
    ...
    let data = {
      ...
      id_beli_obat: this.state.id_beli_obat,
      id_detil_beli_obat: this.state.id_detil_beli_obat,
    ...
    const result = await this.props.dispatch(update_purchase_medicine(data))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PURCHASE_MEDICINE_DETAIL)
    }else {
      this.showError(result.getMessage())
  ...
  async get_detail_data() {
    let response = await this.props.dispatch(
      get_detail_purchase_medicine_data(this.id)
    )
    if(response.isSuccess()) {      
      ...
      this.setState({ 
        id_detil_beli_obat: data.id_detil_beli_obat, id_beli_obat: data.id_beli_obat, id_obat: data.id_obat, nama: data.nama, fungsi: data.fungsi, satuan: data.satuan, jenis: data.jenis, komposisi: data.komposisi, supplier: data.supplier, jumlah: data.jumlah, harga: data.harga, subtotal: data.subtotal, keterangan: data.keterangan
      })
    } else {

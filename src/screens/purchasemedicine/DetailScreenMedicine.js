...
import { delete_medicine_data, delete_purchase_medicine_data, get_detail_medicine_data, get_detail_purchase_medicine_data } from '../../store/actions/medicineAction'

class DetailScreenMedicine extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},            
  ...
  render() {     
    Logger.log('check permission',this.state.detail)
  ...
  async saveInput(data) {
    if(data == 'EDIT') {
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PURCHASE_MEDICINE_UPDATE, {
        itemId: this.id
      })
    } else if(data == 'DELETE') {
      this.delete()
  ...
  async delete() {
    const result = await this.props.dispatch(delete_purchase_medicine_data(this.id))
    if (result.isSuccess()) {
      this.showError(result.getMessage())
      this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PURCHASE_MEDICINE)
    }else {
      this.showError(result.getMessage())
    }
  }
...
DetailScreenMedicine.navigationOptions = ({ navigation }) => ({
  title: 'DETAIL DATA',

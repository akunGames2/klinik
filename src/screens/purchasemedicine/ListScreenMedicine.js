...
import { get_list_medicine_data, get_list_purchase_medicine_data } from '../../store/actions/medicineAction'

class ListScreenMedicine extends BaseScreen {
  constructor(props) {
    super(props)
    this.state = {
      permission: {},
      ...
      return (
        <View style={[Styles.outerContainer,{backgroundColor: 'white'}]}>
          ...
          {this.state.permission && this.state.permission.create && <FloatingActionButton 
            navigation={() => this.props.navigation.navigate(NavKey.MANAGEMENT_DATA_PURCHASE_MEDICINE_ADD)}
            name="add"
            iconType="Ionicons"
          />}
        </View>
  ...
  async onRefresh() {
    ...
    const response = await this.props.dispatch(
      get_list_purchase_medicine_data()
    )
...
ListScreenMedicine.navigationOptions = ({ navigation }) => ({
